...
export async function participantsUpdate({ id, participants, action }) {
    ...
    switch (action) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*𝙂𝙧𝙪𝙥𝙤 𝙂𝙚𝙣𝙞𝙖𝙡 | 𝘾𝙤𝙤𝙡 𝙂𝙧𝙤𝙪𝙥 😼*') :
                            ...
                            this.sendHydrated(id, text, groupMetadata.subject, apii.data,                                                       'https://github.com/Jose21NC/NISHIYIMABot.git', '𝙶𝙸𝚃𝙷𝚄𝙱', null, null, [
                            [(action == 'add' ? '💖𝐒𝐄 𝐔𝐍𝐈𝐎🤗' : '😔𝐒𝐄 𝐒𝐀𝐋𝐈𝐎'), '.s'],    
                            ['𝐌𝐄𝐍𝐔', '/menu']
                            ], '', { mentions: [user]})
                           }
                }
        ...
        case 'quitaradmin':
            if (!text)
                text = (chat.sDe || this.sde || conn.sde || '@user ```is no longer Admin```')
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        unreg: '╰⊱⚠️⊱ *𝘼𝘿𝙑𝙀𝙍𝙏𝙀𝙉𝘾𝙄𝘼 | 𝙒𝘼𝙍𝙉𝙄𝙉𝙂* ⊱⚠️⊱╮\n\n_*¡Qué esperas para estar Verificando(a) en 𝗡𝗶𝘀𝗵𝗶𝗺𝗶𝘆𝗮𝜷𝛔𝞽-MD! Usa el comando #verificar*_\n_*What are you waiting for to be Verifying with 𝗡𝗶𝘀𝗵𝗶𝗺𝗶𝘆𝗮𝜷𝛔𝞽-MD! Use the #verify command*_',
